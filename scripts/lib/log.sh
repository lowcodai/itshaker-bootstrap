...
if [[ -t 1 ]]; then
  _CLR_RESET="\033[0m"
  _CLR_INFO="\033[0;34m"    # Blue
  _CLR_SUCCESS="\033[0;32m" # Green
  _CLR_WARN="\033[0;33m"    # Yellow
  _CLR_ERROR="\033[0;31m"   # Red
  _CLR_SKIP="\033[0;90m"    # Gray
...
# Execution wrapper: honors --dry-run and --verbose
...
# Same, but always prints (even without --verbose)
run_cmd_visible() {
  log_info "$ $*"
  if [[ "${DRY_RUN:-false}" == "true" ]]; then
    log_dry "$*"
